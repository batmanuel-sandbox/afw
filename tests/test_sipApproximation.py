...
from lsst.afw.geom import (Point2D, Point2I, Extent2D, Extent2I, Box2D, Box2I, LinearTransform,
...
def extractCtorArgs(md):
    ...
    kwds = {
        ...
        "cd": LinearTransform(np.array([[md["CD1_1"], md["CD1_2"]],
                                        [md["CD2_1"], md["CD2_2"]]])),
    }
    return kwds
...
class SipApproximationTestCases(lsst.utils.tests.TestCase):
    ...
    def testSipDefinitions(self):
        ...
        def run(md):
            ...
            approx = SipApproximation(a=a, b=b, ap=ap, bp=bp, gridDimensions=gridDimensions, **kwds)
            ...
            bbox = kwds["bbox"]
            pix1 = [Point2D(x, y)
                    for x in np.linspace(bbox.getMinX(), bbox.getMaxX(), gridDimensions.getX())
                    for y in np.linspace(bbox.getMinY(), bbox.getMaxY(), gridDimensions.getY())]
